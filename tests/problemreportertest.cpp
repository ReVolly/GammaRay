...
private slots:
    ...
    void testProblemModel()
    {
        ...
        QCOMPARE(index.data(ProblemModelRoles::SeverityRole).value<Problem::Severity>(), Problem::Warning);
    ...
    void testMetaTypeChecks()
    {
        ...
        QVERIFY(std::any_of(problems.begin(), problems.end(),
            [&obj](const Problem &p){
                return p.problemId.startsWith("com.kdab.GammaRay.MetaObjectBrowser.QMetaObjectValidator")
                       ...
                       && p.description.contains(QStringLiteral("overrides base class property"));
            }
        ));
        ...
        QVERIFY(std::any_of(problems.begin(), problems.end(),
            [&obj](const Problem &p){
                return p.problemId.startsWith("com.kdab.GammaRay.MetaObjectBrowser.QMetaObjectValidator")
                       && p.object == ObjectId(const_cast<QMetaObject*>(obj->metaObject()), "const QMetaObject*")
                       && p.description.contains(QStringLiteral("parameter type not registered"));
            }
        ));
        QVERIFY(std::any_of(problems.begin(), problems.end(),
            [&obj](const Problem &p){
                return p.problemId.startsWith("com.kdab.GammaRay.MetaObjectBrowser.QMetaObjectValidator")
                       && p.object == ObjectId(const_cast<QMetaObject*>(obj->metaObject()), "const QMetaObject*")
                       && p.description.contains(QStringLiteral("property with a type not registered"));
            }
        ));
